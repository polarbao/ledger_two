...
```text
API清单.md  当前 router 实际暴露接口清单、认证要求、账本要求和稳定性标记
...
openapi-v1.3-category-tag-draft.yaml Task53 分类、标签、默认元数据与导入分级自动化契约；Task53.1-Task53.4C 已实现，继续作为 Task53U/Task53.5 验收契约
API规范.md 错误码、分页、筛选、排序、金额、时间和 Ledger Context 规范
```

## 使用规则

1. 新增、删除或修改 API 路径前，必须先更新 `API清单.md`。
2. 新增稳定业务接口前，必须同步更新 `openapi.yaml`。
3. 新增错误码、筛选字段、排序字段或分页行为前，必须同步更新 `API规范.md`。
...
8. v1.2 导入接口以 `openapi-v1.2-import-draft.yaml` 为已实现补充契约，冻结检查需与 `API清单.md` 和 router 同步核对。
9. v1.3 多账本接口以 `openapi-v1.3-ledger-draft.yaml` 为冻结契约；当前实现状态必须与 `API清单.md`、router 和验收测试交叉核对。
