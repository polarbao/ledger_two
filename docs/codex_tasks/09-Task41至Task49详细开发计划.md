...
1. `docs/00-文档索引.md`
2. `docs/prd/24-短中期模块拆解.md`
3. 对应模块 PRD
4. `docs/tech/18-短中期模块架构切片.md`
5. `docs/ui/14-v1.1至v1.2模块流程.md`
6. Task47-Task49 额外读取 `docs/prd/29-v1.2导入模块业务与服务细分.md` 与 `docs/tech/20-v1.2导入模块实施契约.md`
...
状态：已完成；`internal/importer` 已接入 `/api/imports/preview`、`GET /api/imports/{batchID}`、`PATCH /api/imports/{batchID}/rows/{rowID}`，微信/支付宝/通用 CSV fixture、批次持久化、行级调整、Owner/Editor 自有批次权限和前端导入预览工作台已落地。Task47U 移动端验收证据见 `docs/project_analysis/v1.2-task47u-import-workbench-2026-07-08/`。

依赖：

- v1.1 完成。
- 分类、标签、账户管理稳定。
- `docs/prd/29-v1.2导入模块业务与服务细分.md` 已明确 ImportBatch、ImportRow、Parser、Normalizer、Batch、Row、Dedupe、Rule、Commit 服务边界。
- `docs/tech/20-v1.2导入模块实施契约.md` 已确认 API 迁移策略、状态机和 DTO。
...
- 错误提示包含行号和原因。
- invalid 行不得进入可提交状态。
- Owner、Editor 可创建并操作各自的导入批次；Viewer 不可导入，成员间批次互不可见。
...
状态：已完成；Task48 使用独立 `transaction_import_refs` 表保存正式账单与导入行的唯一映射。后端 commit 在单事务内写入正式账单、导入映射、批次状态和审计日志；I02/I03/I04 已补自动化验收，失败批次可在处理问题行后恢复 ready。Task48U 提交确认、结果反馈、事务失败提示和 375px/390px 浏览器验收已完成，证据见 `docs/project_analysis/v1.2-task48-task49-acceptance-2026-07-09/`。
...
协同更新（2026-07-13）：`Task47U/48U/49U` 保留为 v1.2 导入业务和历史验收事实。v1.2 收口后的 Fresh Light 迁移由 `docs/codex_tasks/13-Fresh-Light界面交互协同开发计划.md` 的 `UI-FL-08` 承接，只更新表现层和响应式结构，不重开 parser、去重、规则、commit 或权限范围；发现业务缺陷时回到对应 Task 修复并独立验收。
...
- `docs/ui/figma/v1.1至v1.2与Fresh-Light界面设计稿规范.md`、`docs/ui/figma/组件库规范.md` 已确认。

开发范围：

- 设置页导入入口状态：Owner、Editor 可进入各自的导入工作台，Viewer 显示只读无权限说明；规则管理与商户学习仍仅 Owner 可用。
...
状态：已完成；导入工作台已接入 commit API、提交确认弹窗、导入/跳过/疑似/错误数量二次确认、成功结果反馈和 suspicious 行“确认导入”操作。375px/390px 截图、真实成功提交和事务失败反馈验收已完成；提交弹窗已通过 Portal 修复长页面中的视口定位问题。
...
状态：已完成；导入工作台支持规则创建、编辑、归档、恢复、active/archived/all 筛选和多标签推荐选择，预览行展示命中解释与推荐分类/账户/标签名称；规则引用归档元数据时不会应用建议。Figma 设计稿与真实前端 390px 浏览器验收均已完成，证据见 `docs/project_analysis/v1.2-task48-task49-acceptance-2026-07-09/`。
...
- 规则命中前端状态测试。
- 归档规则和归档元数据回归验收。
- Figma handoff checklist 同步更新，补充 `docs/ui/figma/v1.2-Task49导入规则管理器Figma交接.md`；当前账户主文件为 `https://www.figma.com/design/Q4m7LRw75qrkFdw4O5xmU0`，原文件 `https://www.figma.com/design/wkU5RRZs5R7McjNUlEaFF2` 保留为历史参考。

## 11. Task49：导入规则

状态：已完成；Task49 规则扩展 migration 为 `017_extend_import_rules_for_v12.sql`，后端 `/api/import-rules` 支持 create/list/update/archive/restore，旧 `DELETE` 兼容入口执行归档。active 规则只写 `suggested_*` 与解释字段，不覆盖用户 selected 字段；archived 规则及引用归档元数据的规则不参与预览命中。owner/editor/viewer 权限、移动端规则管理和命中解释均已验收。
...
Task49X 例外说明：2026-07-14 已按真实导出格式纠正为仅新增微信 XLSX；支付宝和通用模板保持 CSV。其范围不得借机扩展到支付宝 XLSX、xls/xlsm、通用 Excel 映射、银行同步或自动提交；详细任务以 `docs/codex_tasks/12-v1.2微信XLSX与支付宝CSV导入专项计划.md` 为准。
