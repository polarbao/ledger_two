...
状态：准备中；Task49 规则扩展 migration 已顺延为 `017_extend_import_rules_for_v12.sql`，在保留旧 `keyword/category_id/account_id/tag_names` 兼容字段的基础上新增 `match_type/pattern/result_json/status/priority/archived_at`。后续功能实现以 v1.2 新字段为主，旧 `/api/import-rules` transitional 行为需要迁移为归档/恢复语义。
