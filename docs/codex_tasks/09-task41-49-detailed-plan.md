...
状态：基础已落地；导入工作台已加入规则管理面板，支持创建、归档、恢复规则，预览行已展示规则命中解释。当前实现先覆盖最小闭环，规则编辑、复杂多标签选择、375px/390px 截图验收和 Figma handoff 仍需继续收口。
