...
Task01-Task49 已完成，Foundation、v1.1 和 v1.2 导入模块已完成实现与验收，当前保持 `1.2.0-rc` 发布收口。Task53 分类标签专项已完成开发前细化并暂缓实现；Task50.3 已整体关闭，下一任务为 Task50.4 active-ledger 与缓存状态机。
...
- 从 Task50.4 继续多账本正式化；Task51 后续只允许开展不冻结范围的 P1 场景与证据准备。
...
- Task50.4 active-ledger、Query Cache 与无账本状态。
...
- Task50.4-Task50.6 多账本正式化。

P3：

- Task53 分类标签智能化实现与 Task51P 在 Task50.6 后重新排序。
