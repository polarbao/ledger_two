...
1. 基于现有 PRD、UI 文档、当前前端页面和 `docs/ui/Fresh-Light视觉参考原型.html`，输出 Figma 配套文件。
...
| `LedgerTwo设计系统方向说明.md` | 设计方向、当前 UI 差异、v1.1 是否调整判断 |
...
| `组件库规范.md` | 组件库规格、状态和前端映射 |
| `v1.1至v1.2与Fresh-Light界面设计稿规范.md` | v1.1/v1.2 逐屏 UI 设计稿规格 |
| `设计开发交接清单.md` | UI 设计与开发交接检查清单 |

已同步更新：

1. `docs/ui/README.md`
2. `docs/ui/15-LedgerTwo长期体验优化专项.md`
3. `docs/00-文档索引.md`
...
3. `Fresh-Light视觉参考原型.html` 的浅色财务方向与现有前端差异较大，直接迁移会影响 AppShell、卡片、抽屉、设置页和移动端断点。
4. v1.1 剩余风险主要是验收证据和 NAS 复核，不是视觉系统不可用。

### 3.2 `Fresh-Light视觉参考原型.html` 的采用方式
