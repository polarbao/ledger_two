...
- `go test ./internal/http/handler/... -count=1` 在 Windows CGO 阶段失败：`go-sqlite3` 目标文件无法被识别为 ELF/Mach-O/PE/XCOFF；不是测试断言失败。
- 当前 WSL VHD 挂载控制路径不可用，无法在本轮重建固定后端镜像；`http://localhost:38088/api/healthz` 仍可返回 `staging / schema 19 / db ok`，但运行实例尚未包含本任务后端修改。

## 5. 发布门禁

在把 UI-FL-05E 标记为“运行验收完成”前，还需在固定镜像中执行 handler 契约测试，并使用真实账号回归普通账单编辑、共同支出重算、归档元数据保留、附件移除、软删除、批量标签和 CSV 导出。数据库不需要 migration，更新镜像时保留现有 schema 19 数据卷。
