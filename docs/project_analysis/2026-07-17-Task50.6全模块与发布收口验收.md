...
验收日期：2026-07-17  
候选提交：`d31b270`
候选镜像：`ledger-two:1.3.0-rc-task50.6-d31b270`
...
数据包按当前角色可见性过滤，不含其他账本、全局 `app_settings`、`instance_admins`、密码或物理附件内容。Owner 的导入原始行还必须属于本人未提交批次，或已关联当前角色可见交易；其他成员 private 交易对应的批次、原始行、审计和历史操作者资料均不导出。Editor 可导出账本快照，但 `import_batches`、`import_items`、`import_rules` 固定为空数组，仅随可见交易保留 `transaction_import_refs` 来源引用。设置页已明确数据包不能替代 SQLite 物理备份或直接恢复。
...
```text
.runtime/v13-task50-6-staging/evidence/browser-20260717T041139/
```
...
4. 当前机器的默认 `Ubuntu` WSL 注册项不可启动；实际 Docker runtime 位于 `Ubuntu-24.04`。本机维护命令必须显式使用 `wsl.exe -d Ubuntu-24.04`，运行中的 38091 容器不受影响。
...
候选镜像 ID 为 `sha256:42d27701d470ebc0b1c4e65d9d2f81099711f613c2df5b1a7ff9089177054631`，revision label 为完整提交 `d31b27074705cfb2ef594f4d73d523c774c413d8`。最终运行复核目录为 `.runtime/v13-task50-6-staging/evidence/20260717-121204/`。
