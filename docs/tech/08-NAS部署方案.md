...
- 备份目录挂载到 `/app/backups`。
- 附件目录挂载到 `/app/uploads`，附件只能通过 `/api/attachments/{filename}` 受保护访问。
- 不把数据库打进镜像。
- 不提交真实 JWT_SECRET。
...
COMPOSE_PROJECT_NAME=ledger-two
...
生产环境会拒绝弱 `JWT_SECRET` 和未显式设置的 `COOKIE_SECURE`。

`APP_ENV` 控制安全校验，NAS staging 与 production 都必须保持 `production`；`DEPLOYMENT_CHANNEL` 用于区分验收数据和正式数据。`IMPORT_XLSX_ENABLED` 在 schema 19 staging 设为 `true`，production 在专项放行前保持 `false`。双环境目录、端口和数据库隔离规则以 `docs/tech/23-v1.2部署环境与数据库隔离.md` 为准。
...
### 8.1 升级
1. **先在页面点击「手动备份」**，确保下载并妥善保管一份最新的 `backup.db`；v1.2 详细检查点见 `docs/releases/v1.2.0-rc-升级与回滚指南.md`。
...
```bash
docker compose exec ledger-two wget -qO- http://localhost:8080/api/healthz
...
**Q4：直接访问 `/uploads/xxx.png` 返回 404？**
...
- 上传附件实际落入挂载的 `uploads/` 目录。
- private 账单附件不能被其他成员或裸 `/uploads/*` 路径读取。
