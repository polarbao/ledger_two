...
`APP_ENV` 控制安全校验，NAS staging 与 production 都必须保持 `production`；`DEPLOYMENT_CHANNEL` 用于区分验收数据和正式数据。双环境目录、端口和数据库隔离规则以 `docs/tech/23-v1.2-deployment-environment-isolation.md` 为准。
