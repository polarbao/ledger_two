...
说明：v1.2 导入模块的业务对象、服务职责、UI 工作台和任务冻结标准以 `docs/prd/29-v1.2导入模块业务与服务细分.md` 为准；最终字段、索引、状态机、DTO、API 迁移和回滚策略以 `docs/tech/20-v1.2导入模块实施契约.md` 为准。本文保留高层切片，不替代 Task47-Task49 的实施契约。
