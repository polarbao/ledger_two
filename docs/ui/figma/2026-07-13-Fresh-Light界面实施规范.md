...
视觉蓝本：`docs/ui/Fresh-Light视觉参考原型.html`<br>
...
1. `AGENTS.md`
2. `docs/00-文档索引.md`
3. `docs/prd/22-v1.1可信赖与高频记账.md`
4. `docs/prd/25-v1.1模块需求规格.md`
5. `docs/prd/29-v1.2导入模块业务与服务细分.md`
6. `docs/tech/18-短中期模块架构切片.md`
7. `docs/tech/19-短中期实施就绪评审.md`
8. `docs/ui/14-v1.1至v1.2模块流程.md`
9. `docs/ui/15-LedgerTwo长期体验优化专项.md`
10. `docs/ui/figma/LedgerTwo设计系统方向说明.md`
11. `docs/ui/figma/v1.1至v1.2与Fresh-Light界面设计稿规范.md`
...
实施边界（2026-07-14）：查看、复制一笔、存为模板和软删除确认已在 UI-FL-05 核心工作台落地；UI-FL-05E 已补齐真正的原账单编辑态。编辑时锁定类型，只允许创建者和可写角色操作，普通附件可调整，共同支出快捷编辑限 `equal/payer_only` 且参与人必须与当前成员一致；离线、自定义分摊、历史成员不一致和 settlement 明确禁用，不得退化成“复制一笔”。详细契约见 `docs/project_analysis/2026-07-14-UI-FL-05E账单编辑契约与验收.md`。
