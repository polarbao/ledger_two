...
本目录用于把 LedgerTwo 的 PRD、技术约束、UI 文档、当前 React 前端和 `docs/ui/Fresh-Light视觉参考原型.html` 原型沉淀为可供 Figma 建模、设计评审、Codex 查阅和前端实现使用的结构化事实源。
...
| 文件 | 角色 | 状态或用途 |
|---|---|---|
| `LedgerTwo设计系统方向说明.md` | 方向要求 | 设计语言、阶段边界和迁移策略 |
| `2026-07-13-Fresh-Light界面实施规范.md` | 实施要求 | Fresh Light 全应用目标、路由映射、任务和禁区 |
...
| `组件库规范.md` | 组件要求 | 组件结构、状态、业务字段和 React 映射 |
| `v1.1至v1.2与Fresh-Light界面设计稿规范.md` | 逐屏要求 | v1.1/v1.2 历史边界和 Fresh Light 目标逐屏规格 |
| `v1.2-Task49导入规则管理器Figma交接.md` | 冻结专项要求 | Task49 规则管理交接规格 |
| `v1.2-Task49X-XLSX导入Figma交接.md` | 冻结专项要求 | Task49X XLSX 导入交接规格与验收状态 |
| `设计开发交接清单.md` | 流程要求 | 每次设计、实现和验收必须满足的交接门槛 |
| `task50-v1.3-multi-ledger/` | 设计要求 | Task50 多账本 Frame Manifest、组件状态与代码交接；线上同步未验证 |
| `task53-v1.3-category-tag/` | 设计要求 | Task53 分类标签智能化 Frame Manifest、组件状态与代码交接；视觉审阅和线上同步未验证 |
...
| 文件 | 角色 | 说明 |
|---|---|---|
| `local-review/2026-07-13-Fresh-Light设计一致性审阅.md` | 人工审阅记录 | 记录文档、Figma 方向和前端实现之间的一致性判断 |
| `local-review/fresh-light-2026-07-13/README.md` | 审阅包说明 | 记录来源、生成方式、内容边界和隐私约束 |
| `local-review/fresh-light-2026-07-13/Fresh-Light预览.html` | 生成审阅文件 | 基于当前 Fresh Light Figma 版本和仓库规范构建的浏览器预览入口 |
...
- UI-FL-01/02 已完成双主题语义层、基础组件、AppShell、全局导航和移动 FAB；页面内容从 UI-FL-03 开始迁移。
...
详细矩阵见 `local-review/2026-07-13-Fresh-Light设计一致性审阅.md`。
...
- 本地事实源：`2026-07-13-Fresh-Light界面实施规范.md`
...
1. 阅读 `LedgerTwo设计系统方向说明.md`，理解阶段边界。
2. 阅读 `2026-07-13-Fresh-Light界面实施规范.md`，确认全应用目标和代码禁区。
3. 阅读 `../../codex_tasks/13-Fresh-Light界面交互协同开发计划.md`，确认波次、共享组件归属和与业务 Task 的协调关系。
4. 使用 `ledger-two.design-tokens.json` 和 `ledger-two.figma-variables.json` 建立变量。
5. 按 `ledger-two-frame-manifest.json` 建立页面和 Frame。
6. 使用 `组件库规范.md` 建立或映射组件。
7. 使用 `v1.1至v1.2与Fresh-Light界面设计稿规范.md` 和专项 handoff 完成逐屏状态。
8. 开发前使用 `设计开发交接清单.md`。
9. 完成后将截图、CDP 指标或未验证原因写入 `local-review/` 或 `docs/project_analysis/`。
10. Task50 多账本设计读取 `task50-v1.3-multi-ledger/README.md` 与独立 manifest，不把未来 Frame 计入 v1.2 Fresh Light 29 Frame 已验收基线。
11. Task53 分类标签设计读取 `task53-v1.3-category-tag/README.md` 与独立 manifest；Task53 required Frame 同样不计入 v1.2 已验收基线。
